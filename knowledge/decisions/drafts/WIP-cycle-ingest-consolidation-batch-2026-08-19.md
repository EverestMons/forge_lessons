...
> 1b. **BATCH FINGERPRINT** — sha256 of `"\n".join(<would-insert headings in parse order>)` == **`4484828a0a400696a9148b89a422cffcbd2443be1a8df81df3e06691621fd34c`**. Mismatch → HALT.
> ⚠️ **The would-insert SET must be derived exactly this way, or the fingerprint is not reproducible:** parse the register, read `SELECT content_hash FROM lesson_entries` from the live corpus read-only, and take the parsed entries **whose `content_hash` is absent from that set, in parse order**. Join their `source_heading` values with `\n` and hash. *(w9-1: the derivation was unstated. The dry run returns COUNTS, not headings, so an agent had no defined way to obtain the input to a HALT-bearing pin — and a plausible alternative such as filtering by `entry_date` yields a different set and a different hash.)* **RESUME with `inserted == 0` → SKIP, record `FINGERPRINT SKIPPED (post-ingest resume)`.**
...
> ⚠️⚠️ **EVERY gate that reads the DB runs on a FRESH read-only connection opened by absolute path — never the writing connection.** G1, G2, G3 and G5 all read live state, and a read on the writing connection sees the **uncommitted** transaction and returns the expected value whether or not the commit landed. **w7-1 put this requirement on Step 1b's post-conditions and nowhere else — G3 asserts the very same `E == N2` and was left exposed.** *(w8-1: the fix landed where the defect was noticed and not at the gates that act on the same property — the third instance of that shape in this cycle.)*
...
> **G4 — `updated` was 0:** assert it from **PERSISTED STATE, not the returned dict** — `SELECT COUNT(*) FROM lesson_entries WHERE id <= <E0> AND ingested_at > '<ingest-start timestamp>'` → **0**. ⚠️ **The dict is an in-memory value that dies with Step 1's process, so Step 2 could only ever re-assert it by trusting Step 1's report** — which is exactly the agent-summary-as-evidence failure QA exists to prevent. The UPDATE branch sets `ingested_at = now` (`:160–168`), so a pre-existing row bearing a post-ingest timestamp is direct proof an update occurred. Measured at walk 8: the newest `ingested_at` among existing rows is `2026-08-15T14:39:31`, comfortably before any dispatch, so the probe is unambiguous. *(w8-2.)* Also re-assert `updated == N3` from the dict **in Step 1 only**, as a cross-check. ⚠️ **This clause previously added "and confirm no pre-existing `content_hash` changed", which had NO probe** — the baseline captures no hash manifest, so it was unverifiable as written. It is also **redundant once traced**: the only write path to an existing row's `content_hash` is the UPDATE branch at `lessons_forge.py:160–168`, and that branch increments `updated`. **`updated == 0` IS the guarantee**, not a proxy for it. *(w5-2.)*
