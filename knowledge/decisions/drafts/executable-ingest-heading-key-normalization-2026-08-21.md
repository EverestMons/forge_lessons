...
- Weak spots (1.4):   w2 1 folded — instruction 1 (W2-1 site 3's justification was WRONG in a way that changes the implementation: it reads `source_heading` from the DB, and once site 1 canonicalizes on INSERT the DB never holds markers — so the helper is the identity there on both existing and future rows. Restated as defence-in-depth with an explicit bound against normalizing a fourth site).
- Destruction (2.4):  w2 dry — the fix's blast radius is bounded by the walk-0 measurement that all 370 existing `source_heading` values contain no markers, so the normalizer is the identity on today's data and the migration is a no-op; the canary proves it against a copy before anything touches the live corpus.
- Vulnerabilities:    w2 dry — traced the annotated-heading path end to end for gaps: `entry_date` is derived from the heading's leading date and markers sit at the END, so it is unaffected; `tags` come from a `**Tags:**` BODY line, not the heading; `content_hash` is body-only. No fourth site was found, which is why the fold above bounds the search rather than inviting it.
- Integration-record: w2 dry — Step 1's renumbering after the walk-1 insertion is contiguous (1-7); the three call sites in Context, the three in Step 1, and the three in the walk-1 record all agree.
...
4. Apply it at **site 3** (`:375-378`) as DEFENCE IN DEPTH, and understand why it is not strictly required: site 3 reads `source_heading` FROM THE DB, and once step 2 canonicalizes on INSERT the DB never holds markers — so on both existing rows (no markers) and future rows (canonicalized) the helper is the identity. Apply it anyway so the derivation is correct for any row that reaches it by another path, but ⚠️ do NOT let this justify widening the change: if you find yourself normalizing at a fourth site, stop and report instead.
