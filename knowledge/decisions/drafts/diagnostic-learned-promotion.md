...
⚠️ **The provenance trail is nearly absent and you must not assume it.** Measured at walk 0: only `C` distinct proposal/entry ids are cited across the whole enforcement surface, and `plan_lint.py`, `propagation_check.py` and `cycle_check.py` cite **none**. The mapping is a reading task, not a grep.
