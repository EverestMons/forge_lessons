...
| id | pin | value | probe |
|---|---|---|---|
| D0 | **`A`** — candidate entries 503 mapped to a demonstrated mechanism | **22** | `F` + `R`; Q2's mapping names 23, of which one is a quarantined bare entry and correctly excluded |
...
**Q1 — Reproduce the defect.** Re-derive `A`, `F`, `R`, `V` and list the ids in each. Show that `V` is `F` plus exactly three of `R`, and name which three were promoted and which three were demoted. ⚠️ **State whether 503 gives ANY basis for that split** — read its Q2 mapping notes and its Q4 set arithmetic before answering, and if there is no basis, say so plainly rather than constructing one.

**Q2 — RULE ONE: does PARTLY count as completion?** 503's Q2 instruction says a lesson only partly enforced is not completion. ⚠️ **That instruction was not invented after the fact** — it was folded into 503 at ITS walk 1, before it ran, precisely to stop a partial mechanism reading as completion. So the rule was intended and stated; what failed was applying it. ⚠️ **Do not simply ratify that — TEST it against the six.** For each `R` entry, read the lesson and the mechanism and state what the mechanism does NOT catch. Then answer: is "partly enforced" a third state, or is it `codified`? ⚠️ **Argue it from the six actual cases, not from the definition** — if any of them is partly-enforced only in a trivial or immaterial way, that is an argument for a narrower rule and you should say so.
