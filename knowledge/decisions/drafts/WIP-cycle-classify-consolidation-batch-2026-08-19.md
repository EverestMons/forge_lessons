...
> - **`cycle_date="2026-08-19"`** — a required positional with no default. ⚠️⚠️ **This is the PLAN's cycle date and it is CORRECT even if you dispatch later. Do NOT recompute it from `date` or `date -u`.** *(w10-2: this shop carries a lesson — and an `accepted|codify` proposal, 352 — saying to MEASURE the date at authoring and never inherit it. **Applied mechanically here it gives the wrong answer**: that rule governs a date describing WHEN THE WORK HAPPENS, while this one names WHICH CYCLE the report belongs to, fixed when the batch was assembled. The two rules point opposite ways and the plan did not disambiguate them.)* The remaining hazard is the original one: never copy a date from the lineage — `"2026-08-14"` and `"2026-08-15"` both appear in the parents.
...
> ✅ **THE RECOVERY, named because detection without it is only half a guard:** all three are **tracked in git** (verified at walk 10). If the post-check fires, the recovery is `git -C "$(pwd)" checkout -- reports/<file>` — **no separate backup copy is needed and none should be made.** *(w10-1: the guard detected an overwrite and told the agent nothing about undoing one. The restore point already existed; the plan simply never said so.)*
>
> **Deposits:**
