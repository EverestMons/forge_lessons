...
- Vulnerabilities:    w3 dry — gate surface verified MECHANICALLY: `plan_lint` exit 0 / 8 PASS / 0 FAIL, `## STEP` count 2, Scope ≡ Deposits (2/2, 3/3) and set-equal, and **site-anchored** to the STEP sections, zero hard-coded `== 11` assertions remain after the walk-1 fold. ⚠️ A WHOLE-FILE grep for that token returns **3**, not 0 — this record line, the walk-1 fold note, and the Closing all QUOTE the instruction that was removed. The probe must be scoped to the instruction sections or it false-fails a correct artifact; the same mirror that produced a false-FAIL in the PST v1.2 amendment ([[earn-the-clean-gate-dont-author-it]]).
