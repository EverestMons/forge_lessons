...
**Plan slug:** executable-lessons-forge-cycle-v2-2026-06-03
...
**v2 correction:** the first dispatch (`halted-executable-lessons-forge-cycle-2026-06-03.md`) was stopped at Step 1. `run_full_lessons_cycle` returns `needs_classification` as every entry currently in LESSONS.md (59: IDs 58-116), not the unclassified subset. Only 23 (94-116) lack proposals; 58-93 already hold dispositioned proposals from the 2026-05-27 cycle. This v2 fixes Step 2 to take its work list from the DB (entries with no proposal), never from `needs_classification` verbatim.
...
- `/Users/marklehn/Developer/GitHub/lessons-forge/knowledge/development/cycle-result-2026-06-03.json` — cycle counts for cross-check only. Do NOT classify from its `needs_classification` list (it includes already-classified entries still present in LESSONS.md) — derive the work list from the DB per the Task.

**Task:**

First derive the authoritative work list from the DB — entries that have NO proposal yet. Do NOT loop the cycle JSON's `needs_classification` field; it lists every entry currently in LESSONS.md (already-classified entries 58-93 included), and classifying those would write duplicate `proposed` rows over dispositioned proposals.
...
Classify only those IDs (expected: the 23 new entries 94-116). Cross-check the count against Step 1's reported ingest count and flag any mismatch in the Output Receipt. For each ID:
...
Expected: 25 passed, 0 failed. Any failure -> flag and halt the step.
