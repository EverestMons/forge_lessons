...
> **Preconditions:** Step 1 AND Step 2 Receipts carry PROCEED-values (allowlist). **STEP 0 — dispatch-state determination for THIS step first**, exactly Step 2's three-probe form aimed at `knowledge/development/dev-log-cycle-step-3-2026-08-07.md` (+ the `bellows-preserved/*` branch list; capture exit codes): a hit → RESUME, recover THIS step's `#### Tranche manifest`; absent from all three → FRESH. **Pre-flight (printed tokens — C8):** (1) `UNCLASSIFIED=<n>` — expected **34** on FRESH; any id outside 215–265 → HALT. (2) **Prior-tranche staleability:** read tranche A's recorded created-proposal ids from Step 2's Receipt (**absent/unparseable → HALT; never reconstruct by predicate — C9**) and check `SELECT 'STALE_IN_A=' || COUNT(*) FROM lesson_proposals WHERE id IN (<tranche A's recorded ids>) AND status='stale';` — non-zero → HALT; on a RESUME additionally `STALE_IN_MINE` over this step's own created-so-far ids. (3) whole-corpus `STALE_COUNT` == Step 1's recorded baseline (3). (4) no other in-progress lessons/cycle plan (main-tree glob).
>
> **Scope:**
...
> **Preconditions:** Steps 1–3 Receipts all PROCEED-values (allowlist). **STEP 0 — dispatch-state determination for THIS step first**, Step 2's three-probe form aimed at `knowledge/development/dev-log-cycle-step-4-2026-08-07.md` (+ `bellows-preserved/*`; exit codes captured). **Pre-flight (printed tokens — C8):** (1) `UNCLASSIFIED=<n>` — expected **17** on FRESH; any id outside 215–265 → HALT. (2) **Prior-tranche staleability:** tranches A+B's recorded created-proposal ids from Steps 2–3's Receipts (**either list missing/unparseable → HALT; no predicate fallback — C9**): `STALE_IN_AB` printed token, non-zero → HALT; on a RESUME additionally `STALE_IN_MINE` over this step's own created-so-far ids. (3) whole-corpus `STALE_COUNT` == 3. (4) no other in-progress lessons/cycle plan.
>
...
**Conflicts:** ledger below; C17 opened at walk-1 culmination; no cross-lens conflict has arisen; none of C14–C17 tested by a later lens yet.
**Closing:** not reached — the last event is walk 1's culmination (a fold); ACID and the cold panel are owed; not deposited.
...
**Ledger status:** C1–C17 OPEN. C14–C16 were opened at authoring from the batch's own entries; C17 by walk 1's culmination. None can be reported SATISFIED before the walks that would test them run.
