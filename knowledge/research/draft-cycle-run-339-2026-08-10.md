...
>
> State the register's before-count read from your worktree snapshot and record that you read it there; the Planner measured 10 rows at authoring, and any difference is a reconcile-note rather than a halt. Do NOT re-raise the `get_unclassified_entries` ordering item — it is already open as rows 9 and 10, and this plan depends on that contract (see the tranche map) rather than duplicating it. Note for the Planner that those two rows are byte-identical duplicates, both written by plan 311's own step 6 through this same channel: live register debt, and the dup-append failure mode observed rather than theorised. **Finally, the mandate carries its own observer** (batch entry 302's rule applied to this plan's own instruction): after the append, re-read the register and confirm it gained exactly ONE row, printing the before count, the after count, and the new row's text. Two rows added, or zero, is a channel failure to report rather than silently accept — a check that exists because the failure it looks for is recorded two rows above it in the very file being written.
>
> `#### Prompt Feedback`.
>
> **Deposits:**
