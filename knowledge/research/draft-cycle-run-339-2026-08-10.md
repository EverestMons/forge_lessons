...
> 7. **Set BOTH target fields on every non-`ambiguous` proposal** (`target_layer='governance'` expected; `target_artifact` per your OWN reading — the scout table is guidance with explicit LICENCE TO DISAGREE, Rule 58). Only `route`/`subcategory`/`duplicate_of` stay `None`.
>
...
>
> ⚠️⚠️ **EVERYTHING BELOW THIS POINT IS PROSE, DELIBERATELY — do NOT write it as a list of any kind.** `sanitize_items` matches `^(?:-\s|\d+\.\s)` (`bellows.py:1419`), so **a numbered item is a bullet to the splitter**, and a block carrying two or more of them emits one register row PER LINE. An earlier draft of this very paragraph was folded into a four-item numbered list one lens ago — it would have written four QA instructions into the Forward Register as items. State each of the following as a sentence, never as an enumerated line.
>
> State the register's before-count read from your worktree snapshot and record that you read it there; the Planner measured 10 rows at authoring, and any difference is a reconcile-note rather than a halt. Do NOT re-raise the `get_unclassified_entries` ordering item — it is already open as rows 9 and 10, and this plan depends on that contract (see the tranche map) rather than duplicating it. Note for the Planner that those two rows are byte-identical duplicates, both written by plan 311's own step 6 through this same channel: live register debt, and the dup-append failure mode observed rather than theorised. ⚠️⚠️ **The observer for this mandate CANNOT live in this step, and saying where it does live is the point.** The agent writes the block in its final message; **the daemon parses the transcript and appends to the MAIN-tree register after the step ends**, so there is no post-append moment inside the step and no worktree path that would show the result. An instruction telling this agent to "confirm the register gained exactly one row" is a check with no reachable state — **which is the guard-without-an-observer class of batch entry 302, and an earlier draft of this paragraph committed it while adding an observer for entry 302's own rule.** So: the agent records the before-count and the exact item text it emitted, and **the OBSERVER is the Planner's, at this step's verdict gate** — re-read `lessons-forge/knowledge/FORWARD.md` in the main tree, confirm it gained exactly one row, and confirm that row's text matches what the agent recorded. Two rows added, or zero, is the dup-append mode — reported, never silently accepted.
>
> `#### Prompt Feedback`.
>
> **Deposits:**
