...
>
> **Tranche — manifest-pinned (C17):** FRESH → the first 14 **NON-DEFERRED** ids of the CURRENT work list ascending. ⚠️⚠️ **The qualifier is load-bearing here specifically: G6-deferred ids are entries whose proposals were all staled, so their ids are BELOW 266 and sort FIRST in an ascending list** — an unqualified "first 14" builds this tranche's manifest out of deferred ids. A walk-8 fold asserted Steps 2 and 3 both stated this; Step 3 did not, and the claim was false when written (expected 280–293; the list is authoritative), written + committed as `#### Tranche manifest` BEFORE the first insert. RESUME → the recovered manifest ∩ unclassified, never re-derived from the shrunken list.
>
