...
> 13. ⚠️⚠️ **FLAG (G)'s PRODUCER binds here too (C14 — a rule stated in one producer and not its siblings is a defect in the direction that is missing):** every disposition line carries **`| remedy: mechanism | owner: <named owner or "unnamed">`** or **`| remedy: discipline`**, applying flag (G)'s test to the entry's own `How to apply:` clause, and where the remedy is a mechanism `suggested_action` states that mechanism and its owner in its own words. ⚠️ **This matters MORE here than in tranche A: not one of flag (G)'s nine core entries is in tranche A — four are in tranche B and five in tranche C.** A producer that binds Step 2 alone produces the signal for none of the entries the flag exists for.
>
...
> - ⚠️⚠️ **FLAG (G)'s PRODUCER binds here too (C14), and this tranche carries FIVE of its nine core entries — 297, 301, 302, 305, 306, every one of them already holding an open FORWARD row.** Every disposition line carries `| remedy: mechanism | owner: <named owner or "unnamed">` or `| remedy: discipline`; where the remedy is a mechanism, `suggested_action` states it and its owner. **These five are the batch's whole recurrence argument: noticed, recorded, never built.**
