...
> ```
> Label it **`NT`**, deposit as RAW output. ⚠️⚠️ **Expected on a FRESH run: 45 rows — 42 `accepted|codify` (21 `DRAFTING_CYCLE.md`, 21 `PLANNER_TEMPLATE.md`) and 3 `stale` are NOT in this predicate, so expect exactly 42 rows here.** Restated precisely, because the two counts are easy to conflate: `NT_COUNT` as this predicate computes it is **42** (`proposed`+`accepted`+`ambiguous`); the 45 in the front matter is that 42 plus the 3 `stale`, which belong to neither partition. Report both.
>
...
> 3. **⚠️⚠️ THE GATE-2 QUEUE CHECK — new in this plan, at EVERY classification step.** `SELECT 'Q2_INTACT=' || COUNT(*) FROM lesson_proposals WHERE status='accepted' AND route='codify';` — must equal **42**. Below 42 → HALT naming the missing ids and the pristine `.backup`. ⚠️ **This runs at every tranche because the window is five verdict gates wide and the rows are unprotected by `_TERMINAL_STATUSES`; a single whole-corpus `STALE_COUNT` check cannot distinguish which rows moved.**
...
**Closing:** not reached — draft v1 written to the drafting path only, no lens has run.
...
**Ledger status:** C1–C19 OPEN. C1, C9 and C17 are amended forms of 311's; C18 and C19 were opened at authoring from this plan's own two inverted-premise findings.
