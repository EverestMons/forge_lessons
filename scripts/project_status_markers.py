...
Usage:
    python3 scripts/project_status_markers.py --db data/lessons-forge.db \
        --lessons /path/to/LESSONS.md                     # dry run (default)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--lessons", required=True, help="path to LESSONS.md")
    ...
    if not lessons.is_file():
        print(f"no such file: {lessons}", file=sys.stderr)
        return 3
    if args.db:
        if not Path(args.db).is_file() or os.path.getsize(args.db) == 0:
            print(f"--db {args.db} is missing or 0 bytes (a decoy, not a database)", file=sys.stderr)
            return 3
        db_path = args.db
    else:
